...
class ScanListViewController: UIViewController, UITableViewDelegate, UITableViewDataSource, ScanListImageViewControllerDelegate, UIPickerViewDelegate, UIPickerViewDataSource {
    ...
    func setupPicker() {
        ...
        toolbar.sizeToFit()
        toolbar.items = [
            UIBarButtonItem(title: "Date", style: .plain, target: self, action: #selector(switchToDateFilter)),
            UIBarButtonItem(title: "Part Type", style: .plain, target: self, action: #selector(switchToPartTypeFilter)),
            UIBarButtonItem(barButtonSystemItem: .done, target: self, action: #selector(dismissPicker))
        ]
        filterField.inputAccessoryView = toolbar
        filterField.placeholder = "Select Filter"
    }

    @objc func switchToDateFilter() {
        currentFilterType = .date
        filterPickerView.reloadAllComponents()
    }

    @objc func switchToPartTypeFilter() {
        currentFilterType = .partType
        filterPickerView.reloadAllComponents()
    }
    ...
    func applyFilters() {
        filteredScans = scans.filter { scan in
            let matchesDate = selectedDate == nil || scan["date"] == selectedDate
            let matchesPart = selectedPartType == nil || scan["partType"] == selectedPartType
    ...
    func pickerView(_ pickerView: UIPickerView, numberOfRowsInComponent component: Int) -> Int {
        ...
        case .date: return availableDates.count + 1
        case .partType: return availablePartTypes.count + 1
        }
    }

    func pickerView(_ pickerView: UIPickerView, titleForRow row: Int, forComponent component: Int) -> String? {
        switch currentFilterType {
        case .date: return row == 0 ? "All Dates" : availableDates[row - 1]
        case .partType: return row == 0 ? "All Part Types" : availablePartTypes[row - 1]
        }
    }
    ...
    func pickerView(_ pickerView: UIPickerView, didSelectRow row: Int, inComponent component: Int) {
        switch currentFilterType {
        case .date:
            selectedDate = row == 0 ? nil : availableDates[row - 1]
            filterField.text = selectedDate ?? "All Dates"
        case .partType:
            selectedPartType = row == 0 ? nil : availablePartTypes[row - 1]
            filterField.text = selectedPartType ?? "All Part Types"
        }
        applyFilters()
